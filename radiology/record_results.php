...
if ($orderId) {
    // Get order details
    $order = $conn->query("SELECT ro.*, rt.test_name, 
                          ...
                          WHERE ro.order_id = $orderId")->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_results'])) {
    ...
    $radiologistId = $_SESSION['related_id'];
    
    $stmt = $conn->prepare("UPDATE radiology_orders SET results = ?, status = 'Completed', result_date = NOW() WHERE order_id = ?");
...
<div class="container">
    <h2 class="text-center my-4">Record Radiology Test Results</h2>
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php
                        $query = "SELECT ro.order_id, rt.test_name, ro.order_date,
                                 p.first_name AS patient_first, p.last_name AS patient_last,
                                 d.first_name AS doctor_first, d.last_name AS doctor_last
                                 FROM radiology_orders ro
                                 JOIN radiology_tests rt ON ro.radiology_id = rt.radiology_id
